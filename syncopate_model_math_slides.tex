...
\begin{frame}{Chat Loss Masking}
  NPC training data is normalized as:

  \[
    \{\text{npc},\text{input},\text{output}\}
  \]

  The visible player input stays natural, while the trainer prepends NPC and
  language control tokens internally:

  \[
    [\texttt{<npc>},\texttt{<lang>},\text{input tokens},\text{output tokens},\text{EOS}]
  \]

  The model sees the NPC, reply language, and player prompt, but loss is
  computed only on output tokens:
...
\begin{frame}{Simple Loss Example}
  ...
  If the prompt is \([10,20]\) and the response is \([5,3]\), mask out prompt
  targets:
